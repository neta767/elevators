...
function Elevators() {
  const arrFloors: floor[] = Array.from({ length: 10 })
    .map((e, idx) => ({
      index: idx,
  ...
  const elevatorsList: elevator[] = Array.from({ length: 5 }).map(() => ({
    available: false,
    color: "black",
    floor: 0,
  }));
  ...
  return (
    <div className="elevators">
      ...
      <div className="col">
        {elevatorsList.map((elevator, elevatorIdx) => (
          <div className="row" key={elevatorIdx}>
            {arrFloors.map((floor) => (
              <div className="bg">
                <div className="cell-with-color" key={floor.index}>
                  {floor.index}
                  {elevatorIdx}
      ...
      <div className="row">
        {arrFloors.map((floor) => (
          <div key={floor.index} className="cell">
            <button className={floor.button}>{floor.button}</button>
          </div>
        ))}
      </div>
    </div>
