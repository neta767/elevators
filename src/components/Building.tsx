...
// import sound from "src/assets/bell.mp3";
...
const convertMillisecondsToMinutesSeconds = (milliseconds: number) => {
...
function Building() {
  ...
  function callElevator(floorIdx: number): void {
    // r.current!.style.fill = "green";
    // r.current!.style.transform = `translateY(${-200}px)`;
    // r.current!.style.transitionDuration = `${2000}ms`;
    // r.current!.style.animationTimingFunction = "linear";
    // r.current!.style.zIndex = "2";
    // const []
    setFloorList((cur: floor[]) => {
      return cur.map((floor: floor) =>
        floor.index === floorIdx
          ? {
              ...floor,
              button: "waiting",
              text: convertMillisecondsToMinutesSeconds(
            ...
            {floorList.map((floor) => (
              <div className="bg" key={floor.index}>
                ...
                >
                  ...
                  {elevator.destinyFloor === floor.index && floor.text}
                </div>
                {/* ) */}
              </div>
            ))}
